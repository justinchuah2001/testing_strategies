...
For spec 2, event organiser is responsible for creating an event(function **insert\_event**) and also can create event for others. Other than that, organiser also can update the event(function **update\_event**) at present and future but cannot exceed 2050 as mentioned in specification. Also, since only the owner(organiser) or the attendee who has the writer role of this event can delete, create, update or passing this event to someone else, so the **delete\_events**, **insert\_event**, **update\_event** and **move\_event** function is created which can be accessed by organiser or writer only. Both the **insert\_event** and the **update\_event** function performs similarly such that its input parameters are (api, calendar ID, starting date, ending date, start tme, end time, event location, event name, id, attendees). The user is able to set an event in their desired location whether online or offline in their own desired times. They are then able to make changes/ add attendees to the events. However, there are also specific format that has to be followed while performing insert and update, such as the email format of the attendees, therefore a function called **check\_emailFormat** is created to ensure that it follows the normal email conventions. It is said that only organizers can make changes to the event therefore to make sure that it is infact the organizer a function called **check\_details** is called. If the organizers do want to move their own event to other people to take up the organizer role, there is a function called **move\_event** which is used to move an event into another organizer's.
...
Mocking, Branch Coverage, condition coverage, statement coverage
...
The function test_reminders is tested with mocking. For test_reminders function, we've tested with statement coverage which we insert an event and see whether the output reminder is the same with the details we insert or not. Also, it contains other function like insert_event. This functions was also covered in other test suites, with its own respective test methods.
...
Test suite 4 is created to test whether the attendees can reveive the reminders and also make sure they receive the reminder with the correct details.
